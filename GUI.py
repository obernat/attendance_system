...
class Application(Tk):
    ...
    def login_page(self, subject, download=1):
        ...
        if download:
            self.login_button = Button(self, text="Login",
                                       command=lambda: self.login_download(subject, self.username_entry.get(),
                                                                           self.password_entry.get()))
        else:
            self.login_button = Button(self, text="Login",
                                       command=lambda: self.login_upload(subject, self.username_entry.get(),
                                                                         self.password_entry.get()))
    ...
    def login_upload(self, subject_name, name="none", password="none"):
        ...
        ret_value = isp.upload_routine(teacher.subjects_list[0], name, password)
        if ret_value < 0:
    ...
    def subject_info_page(self, subject_name, group, week):
        ...
        self.clear_frame()
        colors = ["red", "green", "yellow", "black", "gray", "yellow", "blue", 'brown']
        ...
        button = Button(self, text="Select",
                        command=lambda: self.subject_info_page(subject_name, variable.get(), week_variable.get()))
            ...
            for name in self.groups[group]:
                ...
                for j in range(0, 12):
                    if (j == int(week_number) - 1):
                        g = j
                        g = Label(self, text=name + "_" + str(j), bg=colors[self.attendance[name][0][j]],
                                  fg=colors[self.attendance[name][0][j]]
                                  , borderwidth=2, highlightthickness=2, highlightcolor="#37d3ff",
